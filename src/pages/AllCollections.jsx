...
const AllCollectionPage = () => {
  // Create a flattened array of all artworks from all collections
  const allArtworks = Object.values(collectionsData).flat();
  const [currentIndexes, setCurrentIndexes] = useState(allArtworks.map(() => 0));
  ...
  const handleNext = (artIndex) => {
    setCurrentIndexes((prev) =>
      prev.map((current, idx) =>
        idx === artIndex ? (current + 1) % allArtworks[artIndex].images.length : current
      )
    );
  };
  ...
  const handlePrev = (artIndex) => {
    setCurrentIndexes((prev) =>
      prev.map((current, idx) =>
        idx === artIndex
          ? (current - 1 + allArtworks[artIndex].images.length) % allArtworks[artIndex].images.length
          : current
      )
    );
  };
  ...
  return (
    <motion.div className="bg-cover bg-center bg-no-repeat text-black min-h-screen flex flex-col items-center"
      style={{
        backgroundImage:
          "url('https://firebasestorage.googleapis.com/v0/b/project-39ce3.appspot.com/o/IMG_9667.JPG?alt=media&token=b7a1becd-7294-4554-af60-03c5c07c5b93')",
      }}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    >
      <div className="max-w-5xl w-full py-12 px-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {allArtworks.map((art, artIndex) => (
